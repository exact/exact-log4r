...
class TestOutputter < Minitest::Test
  include Log4r

  def test_validation
    assert_raises(ArgumentError) { Outputter.new }
    assert_raises(ArgumentError) { Outputter.new 'fonda', :level=>-10}
    assert_raises(TypeError) { Outputter.new 'fonda', :formatter=>-10}
  end
  def test_io
  ...
  def test_boundaries
    ...
    assert_raises(TypeError) { o.formatter = nil }
    assert_raises(TypeError) { o.formatter = String }
    assert_raises(TypeError) { o.formatter = "bogus" }
    assert_raises(TypeError) { o.formatter = -3 }
    # the formatter should be preserved
    assert(o.formatter.class == Formatter)
  end
  def test_file
    assert_raises(TypeError) { FileOutputter.new 'f' }
    assert_raises(TypeError) { FileOutputter.new('fa', :filename => DEBUG) }
    assert_raises(TypeError) { FileOutputter.new('fo', :filename => nil) }
    assert_nothing_raised {
  ...
  def test_log_methods
    ...
    assert_raises(NameError) { o.warn event }
    assert_raises(NameError) { o.error event }
    assert_raises(NameError) { o.fatal event }
  ...
  def test_only_at_validation
    ...
    assert_raises(ArgumentError) { o.only_at }
    assert_raises(ArgumentError) { o.only_at ALL }
    assert_raises(TypeError) { o.only_at OFF }
    assert_nothing_raised { o.only_at DEBUG, ERROR }
    return # cuz the rest is borked
    # test the methods as before
    event = LogEvent.new(nil,nil,nil,nil)
    assert_raises(NameError) { o.debug event}
    assert_raises(NameError) { o.error event}
    assert_nothing_raised { o.warn event}
    assert_nothing_raised { o.info event}
